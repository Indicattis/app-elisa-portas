...
import { useProducaoAuth } from "./useProducaoAuth";
...
interface UseMeuHistoricoProducaoOptions {
  periodo?: 'hoje' | 'semana' | 'mes' | 'todos';
  setor?: SetorType;
}

export function useMeuHistoricoProducao(options: UseMeuHistoricoProducaoOptions = {}) {
  ...
  return useQuery({
    queryKey: ['meu-historico-producao', user?.user_id, periodo, setor],
    queryFn: async () => {
      if (!user?.user_id) return [];

      const dataInicio = getDataInicio();
      const ordens: OrdemHistorico[] = [];
