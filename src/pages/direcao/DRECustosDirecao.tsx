...
type EditingField = { id: string; field: 'custo' | 'unidade' } | null;
...
export default function DRECustosDirecao() {
  ...
  const fetchItens = async () => {
    try {
      ...
      const { data, error } = await supabase
        .from("estoque")
        .select("id, nome_produto, categoria, unidade, custo_unitario, quantidade")
        .eq("ativo", true)
  ...
  const handleStartEdit = (item: EstoqueItem, field: 'custo' | 'unidade') => {
    setEditingField({ id: item.id, field });
    setEditValue(field === 'custo' ? item.custo_unitario.toString() : item.unidade);
  };
  ...
  const handleSave = async () => {
    ...
    if (field === 'custo') {
      ...
      try {
        const { error } = await supabase.from("estoque").update({ custo_unitario: valor }).eq("id", id);
        if (error) throw error;
      ...
      const unidade = editValue.trim();
      if (!unidade) {
        toast.error("Unidade não pode ser vazia");
        return;
      }
      try {
        const { error } = await supabase.from("estoque").update({ unidade }).eq("id", id);
        if (error) throw error;
  ...
  const totalCusto = filtered.reduce((sum, i) => sum + (i.quantidade * i.custo_unitario), 0);
            ...
            <Table>
              <TableHeader>
                <TableRow className="border-white/10 hover:bg-white/5">
                  ...
                  <TableHead className="text-white/70 text-right">Custo Total</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {filtered.length === 0 ? (
                  <TableRow><TableCell colSpan={6} className="text-center text-white/50 py-8">Nenhum item encontrado</TableCell></TableRow>
                ) : (
                  filtered.map((item, index) => (
                    <TableRow key={item.id} className="border-white/10 hover:bg-white/5">
                      ...
                      {editingField?.id === item.id && editingField.field === 'unidade' ? (
                          ...
                          <button
                            onClick={() => handleStartEdit(item, 'unidade')}
                            className="text-white/70 hover:text-blue-400 transition-colors cursor-pointer"
                          >
                            {item.unidade}
                          </button>
                        )}
                      ...
                      <TableCell className="text-right">
                        {editingField?.id === item.id && editingField.field === 'custo' ? (
                          ...
                          <button
                            onClick={() => handleStartEdit(item, 'custo')}
              ...
              {filtered.length > 0 && (
                <TableFooter className="border-white/10 bg-white/5">
                  <TableRow className="hover:bg-white/5">
                    <TableCell colSpan={5} className="text-right font-bold text-white/80">TOTAL</TableCell>
                    <TableCell className="text-right font-bold text-white">{formatCurrency(totalCusto)}</TableCell>
                  </TableRow>
                </TableFooter>
              )}
            </Table>
