...
export default function ChecklistHistorico() {
  ...
  const { data: tarefas = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("tarefas")
        .select("id, descricao, responsavel_id, updated_at, data_referencia")
        .eq("status", "concluida")
      ...
      // Fetch responsaveis
      const responsavelIds = [...new Set((data || []).map((t: any) => t.responsavel_id).filter(Boolean))];
  ...
  const tarefasFiltradas = useMemo(() => {
    ...
    if (dateRange?.from) {
      ...
      resultado = resultado.filter((t) => new Date(t.updated_at) >= from);
    }
    if (dateRange?.to) {
      const to = new Date(dateRange.to);
      to.setHours(23, 59, 59, 999);
      resultado = resultado.filter((t) => new Date(t.updated_at) <= to);
    }

    return resultado;
  }, [tarefas, filtroResponsavel, filtroBusca, dateRange]);
  ...
  return (
    <MinimalistLayout
      ...
      subtitle={`${tarefasFiltradas.length} tarefa${tarefasFiltradas.length !== 1 ? "s" : ""} concluída${tarefasFiltradas.length !== 1 ? "s" : ""}`}
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            <Select value={filtroResponsavel} onValueChange={setFiltroResponsavel}>
              ...
              <SelectContent className="bg-[#0a1628] border-white/10">
                <SelectItem value="todos" className="text-white/70 focus:bg-white/10 focus:text-white">
                  Todos
                </SelectItem>
                {responsaveis.map((r) => (
          ...
          <div className="space-y-2">
            {tarefasFiltradas.map((tarefa) => (
              ...
              >
                ...
                <span className="flex-1 text-sm text-white/80 truncate">{tarefa.descricao}</span>
                ...
                <span className="text-xs text-white/30 shrink-0">
                  {format(new Date(tarefa.updated_at), "dd/MM/yy", { locale: ptBR })}
                </span>
              </div>
            ))}
          </div>
