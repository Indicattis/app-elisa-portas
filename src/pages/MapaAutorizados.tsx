...
export default function MapaAutorizados() {
  ...
  const fetchAutorizados = async () => {
    try {
      ...
      const {
        data,
        error
      } = await supabase.from('autorizados').select(`
  ...
  const handleBatchGeocode = async () => {
    const autorizadosToGeocode = autorizados.filter(autorizado => autorizado.ativo && autorizado.cidade && autorizado.estado);
  ...
  return <div className="fixed inset-0 w-full h-full">
      ...
      <div className="fixed z-[9999] flex flex-col gap-2" style={{
      top: '70px',
      right: '20px'
    }}>
        <Button onClick={handleBatchGeocode} disabled={batchGeocoding} variant="default" size="sm" className="shadow-lg">
          {batchGeocoding ? <Loader2 className="h-4 w-4 animate-spin mr-2" /> : <RefreshCw className="h-4 w-4 mr-2" />}
          Geocodificar todos
        </Button>
        
        <Button onClick={() => setShowOverlays(!showOverlays)} variant="outline" size="sm" className="shadow-lg bg-background/95 backdrop-blur">
          {showOverlays ? <EyeOff className="h-4 w-4 mr-2" /> : <Eye className="h-4 w-4 mr-2" />}
      ...
      paddingTop: '50px'
        ...
        <AutorizadosMapLeaflet autorizados={autorizados} instalacoes={instalacoes} showOverlays={showOverlays} />
      </div>
    </div>;
}