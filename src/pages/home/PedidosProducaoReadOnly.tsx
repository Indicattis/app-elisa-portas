...
import { Package, RefreshCw, Factory, Clock, ClipboardCheck, Paintbrush, Wrench, CheckCircle2, HardHat, AlertTriangle, ShieldCheck, BoxIcon } from "lucide-react";
...
// noop handlers for read-only mode
const noop = async () => {};

export default function PedidosProducaoReadOnly() {
  ...
  useEffect(() => { setPaginaAtual(1); }, [searchTerm, tipoEntrega, corPintura, mostrarProntos, etapaAtiva]);

  const totalPaginas = Math.ceil(pedidosFiltrados.length / ITENS_POR_PAGINA);
  const indiceInicio = (paginaAtual - 1) * ITENS_POR_PAGINA;
  const pedidosPaginados = pedidosFiltrados.slice(indiceInicio, indiceInicio + ITENS_POR_PAGINA);
                      ...
                      <div className="mb-4 space-y-2">
                        ...
                        <div className="space-y-1">
                          {neoInstalacoes.map((neo) => (
                            <NeoInstalacaoCardGestao key={neo.id} neoInstalacao={neo} viewMode={viewMode} onConcluir={noop} isConcluindo={false} />
                          ))}
                        </div>
                      ...
                      <div className="mb-4 space-y-2">
                        ...
                        <div className="space-y-1">
                          {neoCorrecoes.map((neo) => (
                            <NeoCorrecaoCardGestao key={neo.id} neoCorrecao={neo} viewMode={viewMode} onConcluir={noop} />
                          ))}
                        </div>
                        {pedidosFiltrados.length > 0 && (
                          <h3 className="text-sm font-medium text-white/70 mt-4 mb-2">Pedidos ({pedidosFiltrados.length})</h3>
                          ...
                          <PaginationContent>
                            ...
                            {Array.from({ length: totalPaginas }, (_, i) => i + 1).map((pagina) => {
                              ...
                              if (mostrarEllipsisAntes || mostrarEllipsisDepois) {
                                return <PaginationItem key={pagina}><PaginationEllipsis className="text-white" /></PaginationItem>;
                              }
