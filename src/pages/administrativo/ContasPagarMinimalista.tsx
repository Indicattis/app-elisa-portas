...
const CATEGORIA_COLORS: Record<string, string> = {
  materia_prima: 'bg-blue-500/20 text-blue-400 border-blue-500/30',
  ...
  impostos: 'bg-rose-500/20 text-rose-400 border-rose-500/30',
  salarios: 'bg-emerald-500/20 text-emerald-400 border-emerald-500/30',
    ...
    queryFn: async () => {
      ...
      if (fornecedorIds.length > 0) {
        const { data: fornecedores } = await supabase
          ...
          .select('id, nome')
          .in('id', fornecedorIds);
        (fornecedores || []).forEach(f => { fornecedoresMap[f.id] = f; });
      }
