...
export function AppSidebar() {
  ...
  useEffect(() => {
    groupedTabs.forEach(group => {
      const hasActiveChild = group.children.some(child => location.pathname === child.href || location.pathname.startsWith(child.href + '/'));
  ...
  return <Sidebar collapsible="offcanvas" className="border-r">
        ...
        <div className="flex items-center justify-center px-2 h-12">
          <div className="w-full max-w-[120px] h-12 flex items-center justify-center">
            <Link to="/dashboard" className="cursor-pointer">
              <img src={currentLogo} alt="Elisa Portas" className="w-full h-auto max-h-[50px] min-w-[25px] min-h-[25px] object-contain -translate-y-[10px] hover:opacity-80 transition-opacity" />
            </Link>
          </div>
        </div>
      </SidebarHeader>
      ...
      <SidebarFooter className="p-4">
        {diarioBordoTab && diarioBordoTab.can_access && <button onClick={() => navigate('/dashboard/diario-bordo')} className={cn("w-full flex items-center gap-3 px-4 py-3 rounded-lg", "bg-primary text-primary-foreground font-medium", "hover:bg-primary/90 transition-all duration-200", "shadow-md hover:shadow-lg", isActive('/dashboard/diario-bordo') && "ring-2 ring-primary-foreground/20")}>
