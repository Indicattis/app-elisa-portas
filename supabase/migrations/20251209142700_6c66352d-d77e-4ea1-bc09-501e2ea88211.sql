-- 1. LIMPEZA: Deletar ordens do pedido específico 1ed5836f-9448-4828-88b7-7f2aa8c74a71

-- Deletar pontuações de colaboradores primeiro (dependência de linhas_ordens)
DELETE FROM pontuacao_colaboradores 
WHERE linha_id IN (
  SELECT id FROM linhas_ordens WHERE pedido_id = '1ed5836f-9448-4828-88b7-7f2aa8c74a71'
);
...
DELETE FROM ordens_qualidade WHERE pedido_id = '1ed5836f-9448-4828-88b7-7f2aa8c74a71';
DELETE FROM ordens_pintura WHERE pedido_id = '1ed5836f-9448-4828-88b7-7f2aa8c74a71';
DELETE FROM ordens_carregamento WHERE pedido_id = '1ed5836f-9448-4828-88b7-7f2aa8c74a71';

-- 2. ATUALIZAR FUNÇÃO: Adicionar lógica para excluir ordens quando retornar para 'aberto'

CREATE OR REPLACE FUNCTION public.retroceder_pedido_para_etapa(p_pedido_id uuid, p_etapa_destino text, p_motivo text DEFAULT NULL::text)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- Se voltando para 'aberto', limpar TUDO (incluir exclusão de ordens)
