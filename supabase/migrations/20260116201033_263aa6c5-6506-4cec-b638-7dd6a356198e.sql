...
-- Recreate get_portas_por_etapa with renamed parameters to avoid ambiguity
...
BEGIN
  ...
  SELECT
    ...
    -- Portas soldadas (conta linhas de solda concluídas)
    COALESCE((
      SELECT COUNT(*)
      FROM linhas_ordens lo
      WHERE lo.tipo_ordem = 'solda'
        AND lo.concluida = true
        AND lo.concluida_em::date BETWEEN p_data_inicio AND p_data_fim
    ), 0) AS portas_soldadas,
    
    -- Pedidos separados (conta pedidos com separacao_concluida = true)
    COALESCE((
      SELECT COUNT(*)
      FROM pedidos_producao pp
      WHERE pp.separacao_concluida = true
        AND pp.separacao_concluida_em::date BETWEEN p_data_inicio AND p_data_fim
...
-- Recreate get_desempenho_etapas with renamed parameters to avoid ambiguity
...
BEGIN
  ...
  separacao AS (
    SELECT 
      pp.separacao_concluida_por AS uid,
      COUNT(*) AS qtd
    FROM pedidos_producao pp
    WHERE pp.separacao_concluida = true
      AND pp.separacao_concluida_em::date BETWEEN p_data_inicio AND p_data_fim
      AND pp.separacao_concluida_por IS NOT NULL
    GROUP BY pp.separacao_concluida_por
  ),
  pintura AS (
    SELECT 
      lo.concluida_por AS uid,
