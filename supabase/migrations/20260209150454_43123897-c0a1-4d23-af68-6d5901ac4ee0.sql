...
BEGIN
  ...
  -- 1. Linhas de componentes de produção (soldagem, perfiladeira, separação) que requerem pintura
  FOR v_linha IN
    SELECT 
      lo.id as linha_id,
      lo.estoque_id,
  ...
  -- 2. Linhas de pedido_linhas para porta social que requerem pintura
  FOR v_linha IN
    SELECT 
      pl.estoque_id,
      pl.quantidade,
