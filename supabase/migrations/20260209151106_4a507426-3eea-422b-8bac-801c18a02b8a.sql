-- Fix overload 1: text[] parameter
CREATE OR REPLACE FUNCTION public.retornar_pedido_para_producao(p_pedido_id uuid, p_ordem_qualidade_id uuid, p_motivo text, p_ordens_reativar text[], p_user_id uuid)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- UPSERT: reativar etapa existente em vez de inserir duplicada
...
-- Fix overload 2: jsonb parameter
CREATE OR REPLACE FUNCTION public.retornar_pedido_para_producao(p_pedido_id uuid, p_ordem_qualidade_id uuid, p_motivo text, p_ordens_config jsonb, p_user_id uuid)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
