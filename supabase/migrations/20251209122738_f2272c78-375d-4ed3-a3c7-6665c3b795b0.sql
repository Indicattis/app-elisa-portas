...
DECLARE
  v_etapa_atual text;
  v_etapa_destino_valida boolean := false;
...
BEGIN
  ...
  INSERT INTO pedidos_etapas (pedido_id, etapa, data_entrada, em_backlog, motivo_backlog)
  VALUES (p_pedido_id, p_etapa_destino, NOW(), true, p_motivo_backlog);

  -- Update etapa_atual in pedidos_producao
  UPDATE pedidos_producao
  SET etapa_atual = p_etapa_destino, em_backlog = true, updated_at = NOW()
  WHERE id = p_pedido_id;

  -- Record movement (use user_id column, not realizado_por)
