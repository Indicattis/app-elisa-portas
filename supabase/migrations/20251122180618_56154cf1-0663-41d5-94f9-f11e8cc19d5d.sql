-- Corrigir nome da coluna data_entrega para data_prevista_entrega

-- Corrigir função get_meta_producao_mes
DROP FUNCTION IF EXISTS get_meta_producao_mes();
...
BEGIN
  RETURN (
    ...
    WHERE DATE_TRUNC('month', v.data_prevista_entrega) = DATE_TRUNC('month', CURRENT_DATE)
  );
END;
$$;

-- Corrigir função get_pedidos_com_status_ordens
...
BEGIN
  ...
  SELECT 
    ...
    v.data_prevista_entrega as data_entrega,
    jsonb_build_object(
