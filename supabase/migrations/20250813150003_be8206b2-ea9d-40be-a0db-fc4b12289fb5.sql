...
-- Migrar dados da tabela backup para a nova estrutura (sem atendente_id que não existe)
...
-- Atualizar atendente_id baseado no usuario_id da tabela antiga
UPDATE public.orcamentos 
SET atendente_id = (
  SELECT usuario_id 
  FROM orcamentos_backup 
  WHERE orcamentos_backup.id = orcamentos.id
);
