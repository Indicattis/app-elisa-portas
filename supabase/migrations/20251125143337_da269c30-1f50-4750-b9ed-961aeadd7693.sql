-- Corrigir função para usar 'soldagem' em vez de 'solda' no tipo_ordem
...
BEGIN
  ...
  -- Considera TANTO estoque.setor_responsavel_producao QUANTO categoria_linha
  FOR v_linha IN
    SELECT DISTINCT 
      COALESCE(
  ...
  LOOP
    ...
    IF v_setor = 'soldagem' THEN
      ...
      VALUES (
        ...
        v_linha.quantidade,
        'soldagem',  -- Usar 'soldagem' em vez de 'solda'
        v_linha.tamanho,
        v_linha.largura,
        v_linha.altura,
