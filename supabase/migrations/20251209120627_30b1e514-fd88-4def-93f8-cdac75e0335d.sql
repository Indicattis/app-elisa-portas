-- Recriar função retroceder_pedido_para_etapa com 4 parâmetros
-- Corrigindo o uso de 'ativa' (não existe) para 'data_saida IS NULL'
-- E usando 'pedidos_movimentacoes' ao invés de 'pedidos_etapas_historico'

CREATE OR REPLACE FUNCTION public.retroceder_pedido_para_etapa(p_pedido_id uuid, p_etapa_destino text, p_motivo_backlog text, p_user_id uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  -- Buscar pedido e etapa atual (usando data_saida IS NULL para identificar etapa ativa)
  SELECT pp.etapa_atual, pp.tem_pintura 
  ...
  -- Registrar movimentação ANTES das alterações
  ...
  IF p_etapa_destino = 'aberto' THEN
    ...
    -- Fechar etapas abertas (usando data_saida IS NULL)
