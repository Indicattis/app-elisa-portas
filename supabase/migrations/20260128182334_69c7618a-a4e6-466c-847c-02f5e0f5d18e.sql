-- Atualizar função regenerar_linhas_ordem para suportar qualidade e pintura
...
BEGIN
  ...
  -- Verificar se ordem está concluída
  ...
  -- SOLDAGEM, PERFILADEIRA, SEPARACAO: lógica existente
  IF p_tipo_ordem IN ('soldagem', 'perfiladeira', 'separacao') THEN
    ...
    LOOP
      INSERT INTO linhas_ordens (
        pedido_id, ordem_id, tipo_ordem, pedido_linha_id, estoque_id,
        produto_venda_id, item, quantidade, tamanho, largura, altura,
        tipo_pintura, cor_nome, concluida
      ) VALUES (
        v_pedido_id, p_ordem_id, p_tipo_ordem, v_linha.pedido_linha_id,
  ...
  -- QUALIDADE: todas as linhas EXCETO separação
