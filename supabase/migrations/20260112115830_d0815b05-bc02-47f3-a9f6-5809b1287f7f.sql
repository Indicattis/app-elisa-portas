-- Fix: Delete ordens_carregamento by pedido_id before deleting pedidos_producao
...
BEGIN
  ...
  IF v_pedido_id IS NOT NULL THEN
    ...
    -- Excluir ordens_carregamento pelo pedido_id (tem FK para pedidos_producao)
    DELETE FROM ordens_carregamento WHERE pedido_id = v_pedido_id;
    
    -- Excluir instalacoes pelo pedido_id (tem FK para pedidos_producao)
    DELETE FROM instalacoes WHERE pedido_id = v_pedido_id;
  ...
  -- Excluir registros diretos da venda (que ainda não foram excluídos acima)
