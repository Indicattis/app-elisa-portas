-- Corrigir função para usar apenas pontuacao_producao (pontuacao_por_metro não existe)
  ...
  LOOP
    ...
    IF v_estoque_id IS NOT NULL THEN
      ...
      IF v_pontuacao_producao > 0 THEN
        ...
        -- Calcular pontos (apenas por unidade, sem pontuação por metro)
        v_pontos_unidade := v_linha.quantidade * v_pontuacao_producao;
        v_pontos_metro := 0;
        v_pontos_total := v_pontos_unidade;
