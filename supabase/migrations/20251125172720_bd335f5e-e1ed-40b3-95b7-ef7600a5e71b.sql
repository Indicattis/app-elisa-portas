-- Atualizar função retroceder_pedido_para_etapa para tratar ordens de carregamento
...
BEGIN
  ...
  -- Lógica de retrocesso por etapa destino
  IF p_etapa_destino = 'aberto' THEN
    RAISE LOG '[retroceder] CASO 1: Excluindo TUDO (ordens, linhas, instalações, entregas, carregamentos)';
  ...
  ELSIF p_etapa_destino = 'em_producao' THEN
    RAISE LOG '[retroceder] CASO 2: MANTER ordens de produção base, EXCLUIR qualidade, pintura e carregamento';
  ...
  ELSIF p_etapa_destino IN ('aguardando_instalacao', 'aguardando_coleta') THEN
    -- CASO ESPECIAL: Reativar ordem de carregamento em vez de excluir
    RAISE LOG '[retroceder] CASO ESPECIAL: Reativando ordem de carregamento para pedido %', p_pedido_id;
    ...
    -- Resetar check_coleta das linhas do pedido
    UPDATE pedido_linhas
    SET check_coleta = false
    WHERE pedido_id = p_pedido_id;

  ELSE
    -- Para outras etapas, excluir ordem de carregamento
    DELETE FROM ordens_carregamento WHERE pedido_id = p_pedido_id;
  END IF;
